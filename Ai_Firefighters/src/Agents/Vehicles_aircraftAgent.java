...
public class Vehicles_aircraftAgent extends Agent {
	 
		private static final long serialVersionUID = 1L;
		private boolean isAvailable = true;
		private int coordX = 50;
		private int coordY = 50;
		private int waterTank = Configurations.AIRCRAFT_MAX_WATER_TANK_CAPACITY;
		private int fuelTank = Configurations.AIRCRAFT_MAX_FUEL_TANK_CAPACITY;
		...
		protected void setup() {
			...
			addBehaviour(sb);
			System.out.println("Vehicle Firetruck agent started");
		}
			...
			protected void onTick() {
				if(travelingToWater && travelingToFuel) {
					...
					if(coordX == this.destX && coordY == this.destY) {
						fuelTank = Configurations.AIRCRAFT_MAX_FUEL_TANK_CAPACITY;
						this.travelingToFuel = false;
					}
				}
					...
					this.destY = 30;
					if(coordX == this.destX && coordY == this.destY) {
						waterTank = Configurations.AIRCRAFT_MAX_WATER_TANK_CAPACITY;
						this.travelingToWater = false;
					}
				}
			...
			public void action() {
				...
				if(isAvailable) {
					...
					try {
						...
						StatusMessage sm = new StatusMessage(coordX, coordY, fireId, myAgent.getAID(), isAvailable, "AIRCRAFT", fuelTank, waterTank);
				...
				try {
					...
					case (ACLMessage.REQUEST):
						if(content instanceof FireMessage) {
							...
							StatusMessage sm = new StatusMessage(coordX, coordY, fm.getFireId(), myAgent.getAID(), isAvailable, "AIRCRAFT", fuelTank, waterTank);
